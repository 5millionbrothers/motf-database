...
4. 공판장 결제도 같은 방식으로 확인한다.
5. 결제 취소·환불 정책과 운영자 처리 화면을 설계한 뒤 토스 운영 키 전환을 검토한다.
6. 카카오 로그인은 이용자 인증 흐름이 안정된 뒤 붙인다.
7. 카카오 챗봇은 FAQ, 예약 상태 조회, 문의 연결 순서로 붙인다.
