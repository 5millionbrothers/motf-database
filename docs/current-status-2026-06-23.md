...
- 거절된 선결제 요청은 `refund_status = required`로 표시되어 나중에 토스 자동환불 API가 처리할 수 있다.

## Supabase SQL 기준

운영 DB 기준 마이그레이션은 `01`부터 `24`까지다.
...
4. 사장님 거절 시 `refund_status = required`가 남는지 확인하고 토스 결제 취소 API를 연결한다.
