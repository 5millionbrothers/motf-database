# [역사 문서] moTF 2026-06-23 상태 메모

> 현재 운영 기준이 아닙니다. 최신 내용은 `current-status-2026-08-29.md`와 `toss-payment-rollout.md`를 확인하세요.
...
운영 DB 기준 마이그레이션은 `01`부터 `27`까지다.

- `21_toss_payment_intents.sql`: 결제 대기 원장과 승인 후 거래 생성의 초기 기반
...
- `27_portone_payment_hardening.sql`: 포트원 결제 금액 검증 보강과 거래 중복 방지 인덱스 추가
...
3. 포트원 심사 통과 후 KG이니시스 가상계좌 발급, 입금대기 표시, 웹훅 입금완료 생성을 확인한다.
4. 사장님 거절 시 결제 완료 건만 `refund_status = required`가 남는지 확인하고 환불 처리 방식을 확정한다.
5. 공판장 결제도 같은 방식으로 확인한다.
6. 결제 취소·환불 정책과 운영자 처리 화면을 설계한 뒤 포트원 운영 환경 전환을 검토한다.
